...
const validationSchema = toTypedSchema(
  z.object({
    name: z
      .string({ required_error: "Genre category name is required" })
      .min(2, "Genre name must be at least 2 characters long")
  })
)
...
<template>
  <div class="w-full max-w-lg bg-gray-950 border border-gray-900 rounded-3xl p-6 shadow-2xl text-white">
    <div class="mb-6">
      <h2 class="text-xl font-bold tracking-wide">Add Movie Genre</h2>
      <p class="text-xs text-gray-500 mt-1">Register new film categories and unique attributes to the database system ledger.</p>
    </div>
    ...
    <form @submit.prevent="onSubmit" class="space-y-5">
      ...
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">
          Genre Classification Name <span class="text-red-500">*</span>
        </label>
        <input
          v-model="name"
          type="text"
          placeholder="e.g., Sci-Fi, Psychological Thriller, Anime"
          :class="[
      ...
      <div class="pt-2">
        ...
        >
          {{ isSubmitting ? 'Inserting Category Tag...' : 'Save Genre Class' }}
        </button>
      </div>
