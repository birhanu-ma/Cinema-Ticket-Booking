...
const validationSchema = toTypedSchema(
  z.object({
    name: z
      .string({ required_error: "Star name is required" })
    ...
    photoUrl: z
      .string()
      .url("Please provide a valid profile image URL (e.g., https://...)")
      .or(z.literal('')) 
      .nullable()
      .optional()
  })
  ...
  <div class="w-full max-w-lg bg-gray-950 border border-gray-900 rounded-3xl p-6 shadow-2xl text-white">
    <div class="mb-6">
      <h2 class="text-xl font-bold tracking-wide">
        {{ initialData ? 'Edit Star Profile' : 'Register New Star' }}
      </h2>
      <p class="text-xs text-gray-500 mt-1">
        {{ initialData ? 'Modify this cast member\'s biographical files and parameters.' : 'Add cast member profiles and actor details to your dashboard database.' }}
      </p>
    </div>
    ...
    <form @submit.prevent="onSubmit" class="space-y-5">
      ...
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">
          Star Full Name <span class="text-red-500">*</span>
        </label>
        <input
          v-model="name"
          type="text"
      ...
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">
          Headshot Portrait URL <span class="text-gray-600 text-[10px]">(Optional)</span>
        </label>
        <input
          v-model="photoUrl"
          type="text"
          ...
          :class="[
            'w-full bg-gray-900 border rounded-xl px-4 py-3 text-sm focus:outline-none transition-colors placeholder:text-gray-600',
            errors.photoUrl ? 'border-red-500 focus:border-red-500' : 'border-gray-800 focus:border-lime-400'
          ]"
        />
        <span v-if="errors.photoUrl" class="text-xs font-medium text-red-500 mt-0.5">
          {{ errors.photoUrl }}
        </span>
      </div>
      ...
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">
          Actor Biography Summary <span class="text-gray-600 text-[10px]">(Optional)</span>
        </label>
        <textarea
          v-model="bio"
          rows="4"
      ...
      <div class="pt-2">
        ...
        >
          {{ isSubmitting ? 'Saving Metadata...' : (initialData ? 'Update Star Profile' : 'Save Star Profile') }}
        </button>
      </div>
